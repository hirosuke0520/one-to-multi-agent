...
export function Sidebar({
  ...
  const getThreadTitle = (thread: ContentMetadata) => {
    // First priority: Use sourceText if available (input text)
    if (thread.sourceText && typeof thread.sourceText === "string") {
      const sourceText = thread.sourceText.trim();
      if (sourceText && sourceText.length > 0) {
        return sourceText.length > 40 ? sourceText.slice(0, 40) + "..." : sourceText;
      }
    }

    // Second priority: Use originalFileName if it's a file upload
    if (thread.originalFileName && typeof thread.originalFileName === "string") {
      const fileName = thread.originalFileName.trim();
      if (fileName && fileName.length > 0) {
        // Remove file extension for cleaner display
        const nameWithoutExt = fileName.replace(/\.[^/.]+$/, "");
        return nameWithoutExt.length > 40 ? nameWithoutExt.slice(0, 40) + "..." : nameWithoutExt;
      }
    }

    // Third priority: Try to get title from generated content
