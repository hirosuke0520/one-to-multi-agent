...
export class EnvValidator {
  ...
  static validate(): ValidationResult {
    ...
    if (isProduction) {
      ...
      for (const varName of this.productionRequiredVars) {
        if (!env[varName as keyof EnvConfig]) {
          warnings.push(`Missing ${varName} - Google authentication may not work`);
        }
      }
    }
  ...
  private static validateDatabaseConfig(env: EnvConfig, errors: string[], warnings: string[]) {
    // Check database port
    ...
    if (env.NODE_ENV === 'production' && env.DB_HOST) {
      if (!env.DB_HOST.startsWith('/cloudsql/') && !env.DB_HOST.includes('amazonaws.com')) {
        warnings.push('Production DB_HOST should use Cloud SQL socket path or managed database URL');
      }
    }
  }

  private static validateAuthConfig(env: EnvConfig, errors: string[], warnings: string[]) {
    ...
    if (env.AUTH_SECRET) {
      ...
      if (env.AUTH_SECRET === 'your-nextauth-secret-generate-with-openssl-rand-base64-32') {
        errors.push('AUTH_SECRET is still using the example value. Generate a secure secret.');
      }
    }

    // Check Google OAuth configuration
    if (env.AUTH_GOOGLE_ID === 'your-google-client-id' ||
    ...
    if (env.AUTH_GOOGLE_ID && !env.AUTH_GOOGLE_ID.endsWith('.googleusercontent.com')) {
      warnings.push('AUTH_GOOGLE_ID should end with .googleusercontent.com');
    }
  }

  private static validateStorageConfig(env: EnvConfig, errors: string[], warnings: string[]) {
    ...
    if (env.USE_REAL_AI === 'true') {
      if (!env.GOOGLE_API_KEY || env.GOOGLE_API_KEY === 'your-gemini-api-key-here') {
  ...
  static generateHelpMessage(result: ValidationResult): string {
    ...
    if (result.errors.some(e => e.includes('DB_'))) {
      ...
      help += '3. Run: psql -d one_to_multi_agent -f scripts/setup-database.sql\n\n';
    }
