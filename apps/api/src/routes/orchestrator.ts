...
const orchestrator = new Hono<{ Variables: Variables }>();

const processSchema = z.object({
  sourceType: z.enum(["text", "audio", "video"]),
  content: z.string().optional(),
  targets: z.array(z.string()).min(1),
  // userId is now obtained from auth middleware, not from request body
  profile: z
    .object({
...
orchestrator.use("*", authMiddleware);

orchestrator.post("/process", async (c) => {
  try {
    ...
    if (contentType?.includes("multipart/form-data")) {
      ...
      const userId = c.get("userId") as string; // Get from auth context
      ...
      if (customPromptsRaw) {
        try {
          ...
          return c.json(
            { success: false, error: "Invalid customPrompts payload" },
            400
          );
        }
      }
      ...
      const { sourceType, content, targets, profile, customPrompts } =
        validation.data;
      const userId = c.get("userId") as string; // Get from auth context
