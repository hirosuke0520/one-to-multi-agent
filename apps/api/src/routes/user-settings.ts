...
app.get("/global-character-prompt", async (c) => {
  try {
    ...
    return c.json({
      globalCharacterPrompt:
        prompt || userSettingsService.getDefaultGlobalCharacterPrompt(),
    });
  } catch (error) {
...
app.put("/global-character-prompt", async (c) => {
  try {
    ...
    const savedSettings = await userSettingsService.saveGlobalCharacterPrompt(
      userId,
      prompt
    );
    return c.json({
      message: "Global character prompt saved successfully",
      globalCharacterPrompt: savedSettings.globalCharacterPrompt,
    });
  } catch (error) {
...
app.delete("/global-character-prompt", async (c) => {
  try {
    ...
    return c.json({
      ...
      globalCharacterPrompt:
        userSettingsService.getDefaultGlobalCharacterPrompt(),
    });
  } catch (error) {
  ...
  try {
    ...
    return c.json({
      settings: {
        ...
        globalCharacterPrompt:
          settings.globalCharacterPrompt ||
          userSettingsService.getDefaultGlobalCharacterPrompt(),
...
app.post("/initialize", async (c) => {
  try {
    ...
    return c.json({
      ...
      settings: {
        userId: settings.userId,
        globalCharacterPrompt: settings.globalCharacterPrompt,
        createdAt: settings.createdAt,
        updatedAt: settings.updatedAt,
      },
    });
